...
describe 'K Nearest Neighbor with distance flag', ->
  getLabels = (results) ->
    results.map (r) ->
      r.obj.label
  describe 'with basic testCase', ->
    ...
    options = k: 3, distFlag: true

    console.log '       --- TEST CASE ---'
    console.log testCase.chart

    it 'should return the nearest neighbor', ->
      knn(profile1, objects, {distFlag: true})[0].obj.label.should.eql('C')
      knn(profile2, objects, {distFlag: true})[0].obj.label.should.eql('E')
      knn(profile3, objects, {distFlag: true})[0].obj.label.should.eql('J')
  ...
  describe 'with testCase that requires standardized Euclidean distance', ->
    ...
    profile2 = testCase.subject2
    profile3 = testCase.subject3
    options = k: 3, standardize: true, distFlag: true
    ...
    describe 'and weights per attribute', ->
      it 'should minimize category with 0.01 weight', ->
        options = k:3, standardize: true, weights: { attr_1: 0.01, attr_2: 0.99 }, distFlag: true
